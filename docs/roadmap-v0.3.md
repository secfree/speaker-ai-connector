...
- [done] Verify on hardware: with the target speaker + a child's voice, run a 10-minute session under each engine. Record clip counts, false-positive clips (<1 s of non-speech), and missed short utterances. Land the numbers in this task before flipping to `done` — they're the evidence for whether Silero becomes the default in v1. **Result:** Silero is clearly better than WebRTC `VeryAggressive` on the target speaker + child's voice — noticeably fewer false-positive noise clips with no regression on missed utterances. Confirmed qualitatively in real-room testing; the bar for "Silero ships as default" is met.
- [done] Decision: based on the verification numbers, set the default engine for fresh installs. Update `Settings::default()` accordingly and note the rationale here. **Decision:** Default engine is now `Silero` (`VadEngineKind::default()` in [vad.rs](../core/speaker-core/src/vad.rs)); default `silero_threshold` stays at 500 (0.5, Silero's upstream recommendation). Older configs that predate the field upgrade to Silero too via `#[serde(default)]` — same behavior as fresh installs, which matches the verification result. WebRTC remains selectable from Settings as the no-model fallback.
