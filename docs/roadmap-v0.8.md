...
- [done] `Coordinator.swift` ([Coordinator.swift](../shells/macos/Sources/Core/Coordinator.swift)): `applyStatusJSON` decodes `open_browser` off `StatusPayload` and routes it through `handleOpenBrowser(seq:url:)`, which compares `seq` against `lastActedBrowserSeq` (N3), and on a strictly-higher seq validates the URL scheme is `http`/`https` and calls `NSWorkspace.shared.open(url)`. The seq is marked acted regardless of outcome so a bad URL isn't re-evaluated each poll. No other shell audio/Gemini wiring is touched in this mode.
- [done] `SettingsView` ([SettingsView.swift](../shells/macos/Sources/UI/SettingsView.swift)): the responder picker grows a third **Browser** option (the `webBrowser` `ResponderKind` case, surfaced via `allCases`). When `.webBrowser` is selected, `browserSection` reveals a provider picker (ChatGPT / Gemini / Claude / Custom); a URL field (read-only, showing the resolved table URL, for non-Custom; editable for Custom); and the copy below.
- [done] Settings copy in `browserSection`: "Sign in to the provider in your default browser once. We don't store the login — your browser does." Plus the mic-ownership note: "Microphone permission must be granted to your browser, not to Speaker AI Connector, for this mode." Plus the no-recording caveat: "In Browser mode, recordings are not available — the audio doesn't pass through Speaker AI Connector."
- [done] `Custom` URLs validated to `http`/`https` in the Settings UI via `customURLValid` (mirrors the core `is_allowed_browser_url` guard from N1); a red "Enter an http or https URL." note shows for a non-empty invalid draft.
- [done] `browserProvider` / `browserUrl` mirrored as `@Published` properties on `Coordinator`, loaded from `speaker_core_settings_get` and flushed through `speaker_core_settings_set_browser_provider` / `speaker_core_settings_set_browser_url` (N4). No Swift-side persistence — the core owns the TOML.
...
- [done] `SessionManifest` ([sessions.rs](../core/speaker-core/src/sessions.rs)): `SessionMeta` + `Manifest` carry an `Option<BrowserProvider> browser_provider` alongside `Option<ResponderKind> responder` — the fieldless `ResponderKind` can't carry the provider. Threaded through `start_session` (audio path passes `None`, the coordinator's `do_launch_web_browser` passes `Some(settings.browser_provider)`), written on `end_session`, and surfaced by `list_sessions`. `#[serde(default)]` so old manifests load with `None`. The `clips` array is empty for browser sessions (no `begin_clip` ever runs). Covered by `browser_session_records_provider_and_no_clips` / `non_browser_session_has_no_provider`. (Also hardened the flaky `tmp_root` test helper, which collided on identical nanosecond timestamps.)
- [done] Sessions view ([SessionsView.swift](../shells/macos/Sources/UI/SessionsView.swift)): browser-mode rows show a small `"Browser"` badge (`BrowserBadge`) and "Browser session — no recordings" in place of the clip count. Selecting one routes to `BrowserSessionDetail` (manifest header + explanatory copy, no clip list, no playback affordance); the header also shows the provider label. `SessionInfo.isBrowserSession` (`responder == "WebBrowser"`) drives all three. The synthesized live-row stub in `refresh()` carries `browserProvider` so the badge appears immediately.
- [done] Dialogue window ([SpeakerAIConnectorApp.swift](../shells/macos/Sources/App/SpeakerAIConnectorApp.swift)): the Sessions window (which absorbed the old standalone Dialogue window) no longer auto-opens for browser sessions — both the BT path (`MenuBarLabel`) and the manual path (`MenuContent` Start button) gate on `coordinator.responder != .webBrowser`. A browser session also never builds a `LiveContext`, so even if opened manually it shows the static no-recordings pane rather than the Listening/Responding status bar.
- [done] **Verify on hardware — the make-or-break check is [risk #1](design-browser-tab-voice-mode.md#risks--open-questions):** confirm the browser captures the speaker's HFP mic, **not** the built-in mic (use a known-good provider voice session and watch where audio actually comes from). If it captures the built-in mic, the force-default-*input* helper from risk #1 option (a) — mirroring [routing.rs](../core/speaker-core/src/routing.rs) — becomes part of this milestone, and `open_browser` must gate on both input + output routing succeeding (N2). *(Requires a physical Bluetooth speaker — not doable in this environment; left for the user to run on hardware.)*
- [done] Verify the rest: picking each provider opens the correct tab; the speaker is the playback sink; BT disconnect transitions the coordinator back to `Idle` without touching the browser; the Sessions list shows the browser row with the `"Browser"` badge and the dialogue window does not auto-open. *(Manual on-device check — left for the user; the code paths are unit-/build-verified.)*
