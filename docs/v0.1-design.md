...
- **Mic source.** The child's voice has to come from somewhere. Most Bluetooth speakers are output-only; if the target speaker has no HFP/HSP mic, the Mac's built-in mic is used and the child must be in the same room. Document the constraint; pick a mic source per session (prefer the speaker's mic if it advertises one, else fall back to the default input).
...
- Which mic does the speaker actually expose (A2DP-only vs. HFP/HSP with mic)? Affects whether the Mac's built-in mic is the de-facto input.
- HFP audio is mono 8/16 kHz — does that degrade Gemini Live's STT quality enough to matter for a child's voice?
- Cost ceiling: Gemini Live is metered. Decide whether to expose a per-day cap in settings to avoid runaway bills if the speaker is left on.
- Wake-word / push-to-talk: with no UI, how does the child signal "I'm done"? Likely just open-mic for the duration of the Bluetooth connection, with VAD on the Gemini side for turn-taking.
